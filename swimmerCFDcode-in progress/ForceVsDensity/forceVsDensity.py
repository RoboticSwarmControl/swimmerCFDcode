...
#cell_length=np.multiply([1,1.1,1.2,1.3,1.4,1.5],CL) # Max cell length
cell_length=np.multiply([1,1.1,1.2,1.3,1.4,1.5],CL) # Max cell length
...
try:
	...
	ResultsFile=open("scriptResults_ForceVsDensity.txt","r")
...
while True: #begin the loop that starts each simulation
	...
	try:
		...
		for line in ResultsFile:
			if "iteration" in line:
				...
				i=iteration
			if "cellLength" in line:
				lineList=line.split()
				listLength=len(lineList)-1
				cell_length=[]
				for k in range(0,listLength):
					cell_length.append(float(lineList[k+1]))
			if "force" in line:
				lineList=line.split()
				listLength=len(lineList)-1
				forceResults=[]
	...
	if i>len(cell_length)-1:
		ax[0].scatter((cell_length[0:len(forceResults)]), forceResults)  # Plot some data on the axes.
		ax[1].scatter((cell_length[0:len(forceResults)]), computationTimeResults)  # Plot some data on the axes.
		...
		line2="cellLength {}".format(" ".join(str(e) for e in cell_length))
		...
		newTextFile=line1+"\n"+line2+"\n"+line3+"\n"+line4+"\n"+line5
		textOut=open("scriptResults_ForceVsDensity.txt","w")

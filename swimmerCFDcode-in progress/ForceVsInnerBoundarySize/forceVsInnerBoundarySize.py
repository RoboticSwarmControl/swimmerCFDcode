...
dVector=np.multiply([1,1.1,1.2,1.3,1.4,1.5],d) # Ratio of D to L is constant for all runs
lVector=np.multiply([1,1.1,1.2,1.3,1.4,1.5],l)
...
ax[0].set_title('Inner Boundary Size vs. Force')
ax[0].set_xlabel('Inner Boundary diameter [mm]')
ax[0].set_ylabel('Propulsive Force [N]')
ax[0].grid()
ax[1].set_title('Inner Boundary Size vs. Time')
ax[1].set_xlabel('Inner Boundary diameter [mm]')
...
try:
	...
	ResultsFile=open("scriptResults_ForceVsInnerBoundarySize.txt","r")
...
while True: #begin the loop that starts each simulation
	...
	try:
		...
		for line in ResultsFile:
			if "iteration" in line:
				...
				i=iteration
			if "innerDiameter" in line:
				lineList=line.split()
				listLength=len(lineList)-1
				dVector=[]
				for k in range(0,listLength):
					dVector.append(float(lineList[k+1]))
			if "force" in line:
				lineList=line.split()
				listLength=len(lineList)-1
				forceResults=[]
	...
	if i>len(dVector)-1:
		ax[0].scatter((dVector[0:len(forceResults)]), forceResults)  # Plot some data on the axes.
		ax[1].scatter((dVector[0:len(forceResults)]), computationTimeResults)  # Plot some data on the axes.
		fig.set_size_inches(10,5)
		plt.autoscale()
		fig.savefig('ForceVsInnerBoundarySize'+str(i)+'.png',dpi=200)
		plt.show()
		...
		line2="innerDiameter {}".format(" ".join(str(e) for e in dVector))
		...
		newTextFile=line1+"\n"+line2+"\n"+line3+"\n"+line4+"\n"+line5
		textOut=open("scriptResults_ForceVsInnerBoundarySize.txt","w")

...
ax[0].set_title('Mesh Density vs. Force')
ax[0].set_xlabel('Max cell length [mm]')
ax[0].set_ylabel('Propulsive Force [N]')
ax[0].grid()
ax[1].set_title('Mesh Density vs. Time')
ax[1].set_xlabel('Max cell length [mm]')
...
try:
	...
	ResultsFile=open("scriptResults_ForceVsOuterBoundarySize.txt","r")
...
while True: #begin the loop that starts each simulation
	...
	if i>len(DVector)-1:
		...
		plt.autoscale()
		fig.savefig('ForceVsOuterBoundarySize'+str(i)+'.png',dpi=200)
		plt.show()
		...
		line2="outerDiameter {}".format(" ".join(str(e) for e in DVector))
		...
		newTextFile=line1+"\n"+line2+"\n"+line3+"\n"+line4+"\n"+line5
		textOut=open("scriptResults_ForceVsOuterBoundarySize.txt","w")

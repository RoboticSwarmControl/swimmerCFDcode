...
S_refinement=np.multiply([1,1.1,1.2,1.3,1.4,1.5],sr) # Refinement
D_refinement=np.multiply([1,1.1,1.2,1.3,1.4,1.5],dr) # Refinement
...
ax[0].set_title('Refinement vs. Force')
ax[0].set_xlabel('Swimmer Refinement')
ax[0].set_ylabel('Propulsive Force [N]')
ax[0].grid()
ax[1].set_title('Refinement vs. Time')
ax[1].set_xlabel('Swimmer Refinement')
...
try:
	...
	ResultsFile=open("scriptResults_forceVsRefinement.txt","r")
...
while True: #begin the loop that starts each simulation
	...
	try:
		...
		for line in ResultsFile:
			if "iteration" in line:
				...
				i=iteration
			if "swimmerRefinement" in line:
				lineList=line.split()
				listLength=len(lineList)-1
				S_refinement=[]
				for k in range(0,listLength):
					S_refinement.append(float(lineList[k+1]))
			if "force" in line:
				lineList=line.split()
				listLength=len(lineList)-1
				forceResults=[]
	...
	if i>len(S_refinement)-1:
		ax[0].scatter((S_refinement[0:len(forceResults)]), forceResults)  # Plot some data on the axes.
		ax[1].scatter((S_refinement[0:len(forceResults)]), computationTimeResults)  # Plot some data on the axes.
		fig.set_size_inches(10,5)
		plt.autoscale()
		fig.savefig('ForceVsRefinement.png',dpi=200)
		plt.show()
		...
		cfd.setSwimmerRefinment(int(sr))
		cfd.setRotatingDomainRefinment(int(dr))
		...
		line2="swimmerRefinement {}".format(" ".join(str(e) for e in S_refinement))
		...
		newTextFile=line1+"\n"+line2+"\n"+line3+"\n"+line4+"\n"+line5
		textOut=open("scriptResults_forceVsRefinement.txt","w")
		...
		fig.set_size_inches(10,5)
		plt.autoscale()
		fig.savefig('ForceVsRefinement'+str(i)+'.png',dpi=200)
		plt.show()

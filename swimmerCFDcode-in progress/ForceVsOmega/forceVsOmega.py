...
frequencyVector=[20] # Forward velocities
#frequencyVector=[20,40,60,80,20,40,60,80,20,40,60,80]
...
ax[0].set_title('Angular Velocity vs. Force')
ax[0].set_xlabel('Angular Velocity [Hz]')
ax[0].set_ylabel('Propulsive Force [N]')
ax[0].grid()
ax[1].set_title('Angular Velocity vs. Time')
ax[1].set_xlabel('Angular Velocity [Hz]')
...
try:
	...
	ResultsFile=open("scriptResults_ForceVsOmega.txt","r")
...
while True: #begin the loop that starts each simulation
	...
	if i>len(frequencyVector)-1:
		ax[0].scatter((frequencyVector[0:len(forceResults)]), forceResults)  # Plot some data on the axes.
		ax[1].scatter((frequencyVector[0:len(forceResults)]), computationTimeResults)  # Plot some data on the axes.
		fig.set_size_inches(10,5)
		plt.autoscale()
		fig.savefig('ForceVsOmega'+str(i)+'.png',dpi=200)
		plt.show()
		break #Break loop if computation is complete

	else: #otherwise continue computation
		
		Omega=frequencyVector[i]
		#///////////////////////////////////////////////////////////////////
		...
		line2="rotationalVelocity {}".format(" ".join(str(e) for e in frequencyVector))
		...
		newTextFile=line1+"\n"+line2+"\n"+line3+"\n"+line4+"\n"+line5
		textOut=open("scriptResults_ForceVsOmega.txt","w")
